...
export class AppComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    /*const obs = this.minhaObservable('Torres');
    obs.subscribe(observer) */

    const obs = this.usuarioObservable('Admin', 'torres@gmail.com');
    const subs = obs.subscribe(observer);

    setTimeout(() => {
      subs.unsubscribe();
      console.log('Estado da conexão: ' + subs.closed)
    }, 3500);
  }
}
